...
M.pistonTemperature = 0
M.pistonSurfaceTemperature = 0
...
M.engineWearMult = 1
M.engineWearRate = 0.000001
M.cylinderWallWear = 1
M.pistonWear = 1
M.pistonRingWear = 1
...
local function setPartConditionThermals(odometer, integrity, visual)
  ...
  if type(integrity) == "number" then
    ...
    integrityState = {
	  ...
	  oilCondition = linearScale(integrityValue, 1, 0, 1, 0.2),
	  pistonWear = linearScale(odometer, 30000000, 1000000000*2, 1, 0.7) * integrityValue,
	  pistonRingWear = linearScale(odometer, 30000000, 1000000000, 1, 0.4) * integrityValue,
	  cylinderWallWear = linearScale(odometer, 30000000, 1000000000, 1, 0.6) * integrityValue
    }
  end
  ...
  M.engineBlockTemperature = integrityState.engineBlockTemperature or M.engineBlockTemperature
  M.cylinderWallTemperature = integrityState.cylinderWallTemperature or M.cylinderWallTemperature
  M.pistonTemperature = integrityState.pistonTemperature or M.pistonTemperature
  M.pistonSurfaceTemperature = integrityState.pistonSurfaceTemperature or M.pistonSurfaceTemperature
  M.coolantTemperature = integrityState.coolantTemperature or M.coolantTemperature
  M.oilTemperature = integrityState.oilTemperature or M.oilTemperature
  M.exhaustTemperature = integrityState.exhaustTemperature or M.exhaustTemperature
  
  M.oilCondition = integrityState.oilCondition or M.oilCondition
  M.pistonWear = integrityState.pistonWear or M.pistonWear
  M.pistonRingWear = integrityState.pistonRingWear or M.pistonRingWear
  M.cylinderWallWear = integrityState.cylinderWallWear or M.cylinderWallWear
  
  fluidReservoirs.oil.currentMass = integrityState.oilMass or fluidReservoirs.oil.currentMass
end
...
local function getPartConditionThermals()
  local integrityState = {
	...
	pistonTemperature = M.pistonTemperature,
	pistonSurfaceTemperature = M.pistonSurfaceTemperature,
	coolantTemperature = M.coolantTemperature,
	oilTemperature = M.oilTemperature,
	exhaustTemperature = M.exhaustTemperature,
	oilCondition = M.oilCondition,
	oilMass = fluidReservoirs.oil.currentMass,
	pistonWear = M.pistonWear,
	pistonRingWear = M.pistonRingWear,
	cylinderWallWear = M.cylinderWallWear
  }
...
local function updateExhaustGFX(dt)
  ...
  local afr = parentEngine.lastEffAfr or 14.7
  local stoicAfr = 14.7
  local richness = max((stoicAfr - afr) / (stoicAfr - 10), 0) * 0.5
  if afr > 0 and parentEngine.requiredEnergyType ~= "diesel" then
	particulateEmission = particulateEmission + (0.4 * richness)
  end
...
local function updateMechanicalWear(dt, energyTotal, energyToPistons, energyToCylinderWall)
  local wearRateOil = 1 + ((1 - M.oilCondition)^3) * 5

  local oilDamage = (max(parentEngine.unburntFuel or 0, 0.1) * energyTotal * M.oilTemperature/93) / fluidReservoirs.oil.currentMass
  M.oilCondition = M.oilCondition - oilDamage * M.engineWearRate * 0.002 * M.oilWearMult

  local ringTemp = M.pistonTemperature * 0.5 + M.cylinderWallTemperature * 0.5
  local ringEnergy = energyToPistons * 0.4 + energyToCylinderWall * 0.6

  local normalTemp = 90
  local cylinderWallWearRate = 1+((M.cylinderWallTemperature - normalTemp) / (damageThreshold.cylinderWallTemperature - normalTemp))^2
  local pistonWearRate = 1+((M.pistonSurfaceTemperature - normalTemp) / (damageThreshold.cylinderWallTemperature*2 - normalTemp))^2
  local ringWearRate = 1+((ringTemp - normalTemp) / (damageThreshold.cylinderWallTemperature*1.6 - normalTemp))^2

  local starvation = 1 - (engineThermalData.oilLubricationCoef or 1)
  if starvation > 0.8 then
	starvation = starvation + 1
  end  
  
  local t = M.oilTemperature
  local coldPenalty = math.exp(-((t - normalTemp)^2) / (2 * 40^2))
  local hotPenalty = 1 / (1 + max(t - 120, 0)^2 * 0.002)
  local lubricationEfficiency = coldPenalty * hotPenalty
  
  local oilWearMod = (1 + (starvation^3) * 10) * wearRateOil / lubricationEfficiency
  local wearRate = M.engineWearRate * M.engineWearMult
  
  if M.pistonTemperature > damageThreshold.cylinderWallTemperature * 2 then
    local threshold = damageThreshold.cylinderWallTemperature * 2
    local over = (M.pistonTemperature - threshold) / threshold
    
    pistonWearRate = pistonWearRate * (1 + over^2 * 20)
    
    if over > 0.2 then
  	pistonWearRate = pistonWearRate * (1 + over^4 * 6000)
    end
    
    M.pistonWear = M.pistonWear - over * 0.002 * dt
  end
  
  if ringTemp > damageThreshold.cylinderWallTemperature * 1.6 then
    local threshold = damageThreshold.cylinderWallTemperature * 1.6
    local over = (ringTemp - threshold) / 80
    
    ringWearRate = ringWearRate * (1 + over^2 * 60)
    
    if over > 1.0 then
  	ringWearRate = ringWearRate * (1 + over^3 * 4000)
  	M.pistonRingWear = M.pistonRingWear - over * 0.003
    end
    
    if over > 2.0 then
  	M.pistonRingWear = M.pistonRingWear - over * 0.008
    end
  end
  
  local ringDamage = 1-M.pistonRingWear
  if ringDamage > 0.3 then
  	local severity = (ringDamage - 0.3) / 0.7
  	
  	cylinderWallWearRate = cylinderWallWearRate * (1 + severity^2 * 100000)
  end
  
  if M.cylinderWallTemperature > damageThreshold.cylinderWallTemperature then
    local threshold = damageThreshold.cylinderWallTemperature
    local over = (M.cylinderWallTemperature - threshold) / 60
    
    cylinderWallWearRate = cylinderWallWearRate * (1 + over^2 * 8)
    
    if over > 1.0 then
  	cylinderWallWearRate = cylinderWallWearRate * (1 + over^2 * 40)
  	
  	M.cylinderWallWear = M.cylinderWallWear - over * 0.0005
    end
    
    if over > 2.0 then
  	cylinderWallWearRate = cylinderWallWearRate * (1 + over^3 * 200)
  	
  	M.cylinderWallWear = M.cylinderWallWear - over * 0.0015
    end
  end
   
  M.cylinderWallWear = max(M.cylinderWallWear - energyToCylinderWall * cylinderWallWearRate * energyCoef.cylinderWallSurface * oilWearMod * wearRate * 0.0000525, 0)
  M.pistonWear = max(M.pistonWear - energyToPistons * pistonWearRate * energyCoef.pistonSurface * wearRate * 0.0000525, 0)
  M.pistonRingWear = max(M.pistonRingWear - ringEnergy * ringWearRate * energyCoef.pistonRings * oilWearMod * wearRate * 0.0000525, 0)
end

local function updateAirCoolingGFX(dt)
  tEnv = obj:getEnvTemperature() + conversion.kelvinToCelsius
  
  local pistonCoolingEfficiency = burnEfficiencyCoef[100]*0.6
  
  local oilCooling = (((parentEngine.maxTorque - parentEngine.maxTorqueMod))*parentEngine.outputAV1*0.001)

  --k "spring" values
  local kExhaustToAir = 0.000015
  local kCylinderWallToOil = 60
  local kOilSumpToAir = 200
  local kPistonToOil = 0--(((parentEngine.maxTorque - parentEngine.maxTorqueNA))*parentEngine.maxPhysicalAV*0.001)
  local kOilToBlock = 300
  local kCylinderWallToBlock = 2200
  
  local kPistonSurfaceToPistons = (parentEngine.maxTorqueMod*parentEngine.maxPhysicalAV*0.001)*pistonCoolingEfficiency*5--60 * coolMult
  local kCylinderWallToPistons = (parentEngine.maxTorqueMod*parentEngine.maxPhysicalAV*0.001+oilCooling)*pistonCoolingEfficiency--12 * coolMult

  if M.energyType == "diesel" then kCylinderWallToPistons = kCylinderWallToPistons/1.5 end
  ...
  local wallHeatFraction = (1-(M.cylinderWallTemperature / 1000))^2
  wallHeatFraction = 1

  local energyTotal = burnEnergyPerUpdate + parentEngine.pumpingLossPerUpdate
  local energyWalls = energyTotal * 0.5 * (M.combustHeatMult or 1) * wallHeatFraction--min((M.combustHeatMult or 1), 1-((M.pistonTemperature + M.cylinderWallTemperature)/2 - 90) / (700 - 90))
  local energyExhaust = energyTotal - energyWalls

  local energyToCylinderWall = energyWalls * 0.7
  
  local energyToPistons = energyWalls - energyToCylinderWall
  
  local energyToExhaust = energyExhaust
  local energyToOil = parentEngine.frictionLossPerUpdate

  local energyPistonSurfaceToPistons = (M.pistonSurfaceTemperature - M.pistonTemperature) * kPistonSurfaceToPistons
  local energyCylinderWallToPistons = (M.cylinderWallTemperature - M.pistonTemperature) * kCylinderWallToPistons
  local energyPistonsToOil = (M.pistonTemperature - M.oilTemperature) * kPistonToOil
  ...
  M.cylinderWallTemperature = max(M.cylinderWallTemperature + (energyToCylinderWall - (energyCylinderWallToOil + energyCylinderWallToBlock + energyCylinderWallToPistons) * dt) * energyCoef.cylinderWall, tEnv)
  M.oilTemperature = max(M.oilTemperature + (energyToOil + (energyCylinderWallToOil - energyOilToAir - energyOilSumpToAir - energyOilToBlock + energyPistonsToOil) * dt) * energyCoef.oil, tEnv)
  ...
  M.pistonSurfaceTemperature = max(M.pistonSurfaceTemperature + ((energyToPistons - (energyPistonSurfaceToPistons * dt)) * energyCoef.pistonSurface), tEnv)
  M.pistonTemperature = max(M.pistonTemperature + ((energyPistonSurfaceToPistons + energyCylinderWallToPistons - energyPistonsToOil) * dt) * energyCoef.pistons, tEnv)
  ...
  updateMechanicalWear(dt, energyTotal, energyToPistons, energyToCylinderWall)
  ...
  engineThermalData.oilCondition = M.oilCondition
  engineThermalData.pistonWear = M.pistonWear
  engineThermalData.cylinderWallWear = M.cylinderWallWear
  engineThermalData.pistonRingWear = M.pistonRingWear  
...
local function updateWaterCoolingGFX(dt)
  ...
  local currentEngineEfficiency = burnEfficiencyCoef[math.floor(parentEngine.engineLoad * 100)]
  local burnEnergyPerUpdate = parentEngine.engineWorkPerUpdate * currentEngineEfficiency
  --print(currentEngineEfficiency .. ", " .. 1/currentEngineEfficiency)

  local energyTotal = burnEnergyPerUpdate + parentEngine.pumpingLossPerUpdate
  local energyWalls = energyTotal * 0.5 * (M.combustHeatMult or 1)
  local energyExhaust = energyTotal - energyWalls
  ...
  --Step 2: The integrator
  M.cylinderWallTemperature = max(M.cylinderWallTemperature + (energyToCylinderWall - (energyCylinderWallToOil + energyCylinderWallToCoolant + energyCylinderWallToBlock + energyCylinderWallToPistons) * dt) * energyCoef.cylinderWall, tEnv)
  ...
  M.oilTemperature = max(M.oilTemperature + (energyToOil + (energyPistonsToOil + energyCylinderWallToOil - energyOilToAir - energyOilSumpToAir - energyOilToBlock) * dt) * energyCoef.oil, tEnv)
  ...
  M.pistonSurfaceTemperature = max(M.pistonSurfaceTemperature + ((energyToPistons - (energyPistonSurfaceToPistons * dt)) * energyCoef.pistonSurface), tEnv)
  M.pistonTemperature = max(M.pistonTemperature + ((energyPistonSurfaceToPistons + energyCylinderWallToPistons - energyPistonsToOil) * dt) * energyCoef.pistons, tEnv)
  --print(M.pistonTemperature .. ", " .. M.cylinderWallTemperature .. ", " .. (energyToPistons + energyCylinderWallToPistons))
  ...
  engineThermalData.cylinderWallWear = M.cylinderWallWear
  engineThermalData.pistonRingWear = M.pistonRingWear
  engineThermalData.currentOilMass = fluidReservoirs.oil.currentMass
...
local function updateMechanicsGFX(dt)
  ...
  local ringWear = 1-M.pistonRingWear
  local cylinderWear = 1-M.cylinderWallWear
  local baseOilBurn = M.displacement * 0.0000012
  local oilBurnRate = baseOilBurn * (1 + 200 * ringWear^2 + 2 * cylinderWear) * (0.3 + 0.7 * parentEngine.engineLoad) * ((absEngineRPM) / 3000)

  --if M.pistonRingsDamaged == true then oilBurnRate = oilBurnRate + 0.005 end

  if engineAVAdjustedPistonRingOilLeakRate + fluidLeakRates.oil.gravity > 0 or fluidReservoirs.oil.massInCylinders > 0 or oilBurnRate > 0.0002 then
  ...
  fluidLeakRates.oil.overall = fluidLeakRates.oil.oilpan + fluidLeakRates.oil.radiator + engineAVAdjustedPistonRingOilLeakRate + fluidLeakRates.oil.gravity + oilBurnRate * M.oilWearMult
  ...
  fluidReservoirs.oil.massInCylinders = clamp(fluidReservoirs.oil.massInCylinders + (engineAVAdjustedPistonRingOilLeakRate + fluidLeakRates.oil.gravity + oilBurnRate - fluidLeakRates.oil.combustionToExhaust) * dt, 0, 0.5)
  M.oilInCylinders = fluidReservoirs.oil.massInCylinders
  M.oilBurn = fluidLeakRates.oil.combustionToExhaust
...
local function reset(jbeamData)
  ...
  M.pistonTemperature = startingTemperature
  M.pistonSurfaceTemperature = startingTemperature
...
local function initThermals(jbeamData)
  ...
  M.pistonTemperature = tEnv
  M.pistonSurfaceTemperature = tEnv
  ...
  local energy = jbeamData.requiredEnergyType or "gasoline"
  
  local displacement = parentEngine.maxTorqueNA / 85
  if energy == "diesel" then displacement = parentEngine.maxTorqueNA / 120 end
  
  M.displacement = displacement
  M.pistonMass = (0.65 * displacement) * (1 + 0.12 * math.log(parentEngine.maxTorqueRating / 570))
  M.energyType = energy

  cylinderWallMass = M.pistonMass * 1.8 + M.pistonMass * 0.8
  
  if energy == "diesel" then
	M.pistonMass = M.pistonMass * 1.5
  end
  local pistonCrown = M.pistonMass * 0.2
  if energy == "diesel" then
	pistonCrown = M.pistonMass * 0.45
  end

  --print(pistonMass .. ", " .. cylinderWallMass)

  energyCoef.coolant = 1 / (fluidReservoirs.coolant.currentMass * constants.coolantSpecHeat)
  energyCoef.oil = 1 / (fluidReservoirs.oil.currentMass * constants.oilSpecHeat)
  energyCoef.cylinderWall = 1 / (cylinderWallMass * cylinderWallSpecHeat)
  energyCoef.cylinderWallSurface = 1 / (cylinderWallMass * 0.05 * cylinderWallSpecHeat)
  energyCoef.pistonRings = 1 / (cylinderWallMass * 0.05 * 450)
	
  energyCoef.pistons = 1 / ((M.pistonMass - pistonCrown) * 900)
  energyCoef.pistonSurface = 1 / ((pistonCrown) * 900)
...
local function changeOil()
	fluidReservoirs.oil.currentMass = fluidReservoirs.oil.initialMass
	M.oilCondition = 1
end
M.changeOil = changeOil

local function fillOil(mass)
	local missingOil = fluidReservoirs.oil.initialMass - fluidReservoirs.oil.currentMass
	if mass == nil then
		mass = missingOil
	end
	mass = min(abs(mass), missingOil)
	fluidReservoirs.oil.currentMass = fluidReservoirs.oil.currentMass + mass
	
	local oilRatio = fluidReservoirs.oil.currentMass / max(fluidReservoirs.oil.currentMass + mass, fluidReservoirs.oil.currentMass)
	local newCondition = 1-((1-M.oilCondition) * oilRatio)
	M.oilCondition = newCondition
end
M.fillOil = fillOil

local function repairEngineParts()
	M.pistonWear = 1
	M.pistonRingWear = 1
	M.cylinderWallWear = 1
end
M.repairEngineParts = repairEngineParts
